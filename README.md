...
4. Run the following script to extract the given CVE data
   ...
   # e.g., python3 ./data/scripts/cve_data.py --cve_id CVE-2024-5129 --output_path ./data/cve_data/data.json
   ```
...
4. Run the following command to reproduce the given CVE (e.g., CVE-2024-5129)
   ```bash
   ENV_PATH=.env MODEL=example_run python3 main.py --cve CVE-2024-5129 --json ./data/cve_data/data.json --run-type build,exploit,verify
   ```

5. The final results will be stored in `shared/CVE-2024-5129/`
...
4. Enter CVE-ID (e.g., CVE-2024-4340) in the given field and click `Load CVE`. Now you can navigate through the agent conversations and tool calls and intermediate artifacts for all components of CVE-Genie for the given CVE's reproduction run.